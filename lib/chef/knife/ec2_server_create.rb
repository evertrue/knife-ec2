    ...
    class Ec2ServerCreate < Knife
      ...
      def run
        ...
        begin
          ...
          raise if (tries -= 1) <= 0
          ui.warn("server not ready, retrying tag application (retries left: #{tries}) error: #{e.message}")
          sleep 5
          retry
        end
      ...
      def validate!
        ...
        if active_hosts.include?(locate_config_value(:chef_node_name))
      ...
      def active_hosts
        active = connection.servers.reject do |s|
          %w(terminated shutting-down).include?(s.state)
        end
        active.map { |s| s.tags['Name'] }
      end
